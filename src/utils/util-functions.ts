...
import { TCounter, TGauge, THistogram } from './types/request.type';
import { groupBy, keyBy } from 'lodash';
import * as moment from 'moment';

export const mapHistogram2ts = (histogram: THistogram): TResponseDurRec[] => {
  const { hashMap, time, buckets } = histogram;
  const bucketWidth = buckets[1] - buckets[0];
  const data: TResponseDurRec[] = [];
  Object.entries(hashMap).forEach(([k, v]) => {
    const props = k.split(',').map((s) => s.split(':'));
    const labels = {};
    props.forEach(([k, v]) => {
      labels[k] = v;
    });

    Object.entries(v.bucketValues).forEach(([k, v]) => {
      if (v !== 0)
        data.push({
          created: time,
          machineId: labels['instance'],
          job: labels['job'],
          lowerBound: Number(k),
          upperBound: Number(k) + bucketWidth,
          count: v,
          path: labels['path'],
        });
    });
...
export const mapCounter2ts = (counter: TCounter): TRequestRateRec[] => {
  const { hashMap, time } = counter;
  const counterTs = Object.entries(hashMap).map(([k, v]) => {
    const kvProps = k.split(',');
    const props = kvProps.map((e) => e.split(':'));
    const labels = {};
    props.forEach(([k, v]) => {
      labels[k] = v;
    });
...
export const mapGuage2ts = (guage: TGauge): TResourceUsageRec[] => {
  const { hashMap, time } = guage;
  const gaugeData = Object.entries(hashMap).map(([k, v]) => {
    const kvProps = k.split(',');
    const props = kvProps.map((e) => e.split(':'));
    const labels = {};
    props.forEach(([k, v]) => {
